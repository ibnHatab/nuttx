/****************************************************************************
 * boards/arm/j721e/common/src/j721e_uniqueid.c
 *
...
void j721e_flash_cmd(const uint8_t *txbuf, uint8_t *rxbuf, size_t count);
...
noinline_function locate_code(".ram_code.j721e_flash_cmd")
void j721e_flash_cmd(const uint8_t *txbuf, uint8_t *rxbuf, size_t count)
{
...
/****************************************************************************
 * Name: j721e_uniqueid_initialize
 *
 * Description:
 *   The J721E doesn't have a unique ID, so we load the ID from the
...
void j721e_uniqueid_initialize(void)
{
  uint64_t x;
  uint8_t  txbuf[J721E_FLASH_ID_BUFFER_SIZE];
  uint8_t  rxbuf[J721E_FLASH_ID_BUFFER_SIZE];

  memset(g_uniqueid, 0xac, CONFIG_BOARDCTL_UNIQUEID_SIZE);
  memset(txbuf, 0, J721E_FLASH_ID_BUFFER_SIZE);
  memset(rxbuf, 0, J721E_FLASH_ID_BUFFER_SIZE);
  txbuf[0] = J721E_FLASH_RUID_CMD;

  j721e_flash_cmd(txbuf, rxbuf, J721E_FLASH_ID_BUFFER_SIZE);

  /* xorshift PRNG: */

  x = *(uint64_t *)(rxbuf + J721E_FLASH_ID_BUFFER_OFFSET);
