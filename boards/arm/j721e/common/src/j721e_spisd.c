/****************************************************************************
 * boards/arm/j721e/common/src/j721e_spisd.c
 *
...
#ifndef CONFIG_J721E_SPISD_SLOT_NO
#  define CONFIG_J721E_SPISD_SLOT_NO 0
#endif
...
int board_spisd_initialize(int minor, int bus)
{
  ...
  spi = j721e_spibus_initialize(bus);
  if (!spi)
    {
  ...
  if (bus == 0)
    {
      j721e_gpio_set_pulls(CONFIG_J721E_SPI0_RX_GPIO, true, false);
    }
#endif
  ...
  if (bus == 1)
    {
      j721e_gpio_set_pulls(CONFIG_J721E_SPI1_RX_GPIO, true, false);
    }
#endif
  ...
  ret = mmcsd_spislotinitialize(minor, CONFIG_J721E_SPISD_SLOT_NO, spi);
  if (ret < 0)
    {
      ferr("ERROR: Failed to bind SPI device to MMC/SD slot %d: %d\n",
           CONFIG_J721E_SPISD_SLOT_NO, ret);
      return ret;
    }
 ...
 *   This function is called only from j721e_spi.c.
 *
 * Returned Value:
