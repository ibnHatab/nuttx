/****************************************************************************
 * boards/arm/j721e/common/src/j721e_pwmdev.c
 *
...
int j721e_pwmdev_initialize(int      slice,
                             int      pin_a,
                             int      pin_b,
                             uint32_t flags)
#else
...
{
  int ret;
  struct j721e_pwm_lowerhalf_s *pwm_lowerhalf;
...
#if defined(CONFIG_PWM_NCHANNELS) && CONFIG_PWM_NCHANNELS == 2
  pwm_lowerhalf = j721e_pwm_initialize(slice, pin_a, pin_b, flags);
#else
  pwm_lowerhalf = j721e_pwm_initialize(slice, pin, flags);
#endif
